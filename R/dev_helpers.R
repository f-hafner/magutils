...
#' Default is `tests/testthat/mock_db/`
#' @param f The function with SQL calls to be captured with all options set inside.
#'
#' @return Saves fixtures to `mock_db/dir/` where `dir` is created and defined by dittodb
#' based on name of `production_db`. For instance, "/mypath/database.sqlite"
#' becomes "_mypath_database.sqlite/".
#'
#' @export
capture_mockdb <- function(production_db, mock_db = "./tests/testthat/mock_db/", f) {
